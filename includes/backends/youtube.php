...
include dirname(__FILE__) . "/yt-func.php";
...
if(isset($_POST['mode'])) {
	if(isset($_POST['yt_url'])) {
		switch ($_POST['mode']) {
			case 'add':
				$time = time();
				$result_vars = json_decode(getYouTubeData($_POST['yt_url'],"JSON"),true);
				...
				if($result) {
					if(empty($result_vars)) {
						$msg = "No stream URL could be obtained from " . $_POST['yt_url'] . ".";
						$buttons[1] = "ok";
						$buttons[2] = "add";

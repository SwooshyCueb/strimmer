...
if(isset($_GET['id'])) {
	$query = "SELECT * FROM db_cache WHERE ID=" . $_GET['id'];
	$storage = mysqli_query($mysqli,$query);
	$row = mysqli_fetch_array($storage);
	if(isset($storage)) {
		$query = "DELETE FROM db WHERE ID=" . $_GET['id'];
		$result = mysqli_query($mysqli,$query);
		if(isset($result)) {
			$query = "DELETE FROM db_cache WHERE ID=" . $_GET['id'];
			$result = mysqli_query($mysqli,$query);
			...
			foreach ($playlist as $key => $song) {
				$pos = $key+1;
				if(isset($song)) {
					if(strpos($song, $row['RETURN_ARG1'])) {
